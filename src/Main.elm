...
import Html exposing (Html, a, div, h1, h2, header, main_, nav, span, text)
...
zennDecoder =
    map2 (\title path -> ( title, path )) (list (field "title" string)) (list (field "path" string))


qiitaDecoder : Decoder Data
qiitaDecoder =
    map2 (\title path -> ( title, path )) (list (field "node" (field "title" string))) (list (field "node" (field "linkUrl" string)))
...
view model =
    ...
    , body =
        [ header []
            [ nav []
                (List.map
                    (\section -> a [ href (String.append "#" section.id) ] [ text section.name ])
                    (sections model)
                )
            , h1 []
